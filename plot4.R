...
cons$Voltage<- as.numeric(as.character(cons$Voltage)) ##convert to numeric format
cons$Global_reactive_power<- as.numeric(as.character(cons$Global_reactive_power)) ##convert to numeric format

######### Plots
png(filename = "plot4.png", width = 480, height = 480, units = "px")
par(mfrow=c(2,2)) # combining plots 

######### Plot1
plot(cons$Full_time, cons$Global_active_power, bg="transparent", type = "l", xlab="", ylab = "Global Active Power (kilowatts)")

######### Plot2
plot(cons$Full_time, cons$Voltage, bg="transparent", type = "l", xlab="datetime", ylab = "Voltage")
...
legend("topright", c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), col = c(1, 6, 4), lty = 1, bty = "n") 

######### Plot4
plot(cons$Full_time, cons$Global_reactive_power, bg="transparent", type = "l", xlab="datetime", ylab = "Global_reactive_power")
